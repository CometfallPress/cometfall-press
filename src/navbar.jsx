...
function Navbar(scrollstate) {
    ...
    const [scrollUp, setUp] = useState(scrollstate.scrollstate.sdir)
    const [scrollPos, setScrollPos] = useState(scrollstate.scrollstate.scrollPosition)

    useEffect(() => {
        const still = scrollPos === scrollstate.scrollstate.scrollPosition
        setUp(scrollstate.scrollstate.sdir)
        setScrollPos(scrollstate.scrollstate.scrollPosition)
        if(!scrollUp&&!still) {
            if(open){setOpen(false)}
        }
    }, [scrollPos, scrollstate.scrollstate.scrollPosition, scrollstate.scrollstate.sdir, scrollUp, open])
        ...
        <>
            ...
            <nav className={`${(scrollUp||scrollPos<150||mousePosition.y<0.2)?'top-0':'-top-60'} transition-all sticky text-lg xl:text-xl 2xl:text-3xl 3xl:text-4xl 4xl:text-5xl 5xl:text-6xl font-regular z-50`}>
                ...
                <div className="bg-dark drop-shadow-xl z-30 w-full flex flex-row items-center mx-auto p-1">
                    ...
                    <a href="https://cometfallpress.com/" className="flex items-center unselectable">
